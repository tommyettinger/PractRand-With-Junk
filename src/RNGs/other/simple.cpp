...
//	return state2;
	...
	//b = rotate64(b, 56) + a ^ 0xDB4F0B9175AE2165L;
	//a = (rotate64(a, 3) ^ b);
	//b = rotate64(b, 57) + a ^ 0xBBE0563303A4615FL;
	//a = (rotate64(a, 5) ^ b);
	//b = rotate64(b, 58) + a ^ 0xA0F2EC75A1FE1575L;
	//a = (rotate64(a, 7) ^ b);
	//b = rotate64(b, 59) + a ^ 0x89E182857D9ED689L;
	//a = (rotate64(a, 11) ^ b);
	//return a;

//	length = 64 terabytes(2 ^ 46 bytes), time = 261349 seconds
//		Test Name                               Raw        Processed   Evaluation
...
// This gets no anomalies until 64TB, but then...
// rng=oriole64, seed=0x0
// length= 64 terabytes (2^46 bytes), time= 258066 seconds
//   Test Name                         Raw       Processed     Evaluation
//   [Low1/64]BCFN(2+5,13-0,T)         R= +12.1  p =  5.5e-6   unusual
...
// SoloRandom with fewer ops and some states moved around.
// Passes 64TB with no anomalies. Minimum guaranteed period is 2 to the 64.
// 3 statements that all fit on one (long) line!
// Three 64-bit states, changed using only ARX operations.
// Never tell me the odds!
...
// this passes 16TB at least, and might go a lot further. 
//uint64_t x = a += 0xD1B54A32D192ED03L;
//uint64_t y = b += 0x8CB92BA72F3D8DD7L;
...
uint64_t x = a = a + 0xD1B54A32D192ED03L;// ^ 0xA0F2EC75A1FE1576L;
uint64_t y = b = b + 0x8CB92BA72F3D8DD7L;// ^ 0xC13FA9A902A6328EL;
y = rotate64(y,  3) ^ (x = rotate64(x, 56) + y ^ 0xBEA225F9EB34556DL) + rotate64(x, 37);
x = rotate64(x, 11) ^ (y = rotate64(y, 41) + x ^ 0xA62B82F58DB8A985L) + rotate64(y, 29);
y = rotate64(y, 12) ^ (x = rotate64(x, 43) + y ^ 0x9E3779B97F4A7C15L) + rotate64(x, 59);
x = rotate64(x, 50) ^ (y = rotate64(y, 52) + x ^ 0xF1357AEA2E62A9C5L) + rotate64(y, 23);
return x;
...
// Period is at minimum 2 to the 64, max is much higher (a multiple of 2 to the 64).
				...
				void chill96::walk_state(StateWalkingObject* walker) {
					walker->handle(stateA);
					walker->handle(stateB);
					walker->handle(stateC);
				}
