	...
	namespace RNGs {
		namespace Polymorphic {
			namespace NotRecommended {
				...
				void pcg32_norot::seed(Uint64 s) {
					state = 0;
					raw32();
					state += s;
					raw32();
				...
				Uint32 mo_Cmfr32::raw32() {
					state = ~(2911329625u * state);
					state = rotate32(state, 17);
					return state;
				}
				...
				Uint32 mo_Cmr32::raw32() {
					state = 4031235431u * state;
					state = rotate32(state, 15);
					return state;
				}
				...
				Uint32 mo_Cmr32of64::raw32() {
					state = 38217494031235431ull * state;
					state = rotate64(state, 37);
					return Uint32(state);
				}
				...
				Uint64 mo_Cmres64::raw64() {
					const uint64_t m = state * UINT64_C(0x7CDD0CFB6AD2A499),
							// 0xC5B3CF786C806DF7 does well up to at least 256GB with rotation by 33, no anomalies
							n = state2 * UINT64_C(0x2C40E78566614E13);
					return (state -= rotate64(m, 35)) ^ (state2 -= rotate64(n, 33));
					// ^ (weyl += UINT64_C(0x9E3779B97F4A7C15));
				}
...
//   ...and 801 test result(s) without anomalies
				...
				Uint64 linnormB::raw64() {
					...
					*/
					...
					s = rotate64(s, R);
					uint64_t x = s ^ -X;
					// change X as a parameter to the generator to 0 to disable any bit flips, 1 to flip all, other (32-bit max, assigned to 64-bit) numbers are allowed
				...
				Uint32 zig32::raw32() {
					...
					*/
					...
					// using 19 for the rotation and shift fails earlier...
					//rng=zig32, seed=0x0
					//length= 16 gigabytes (2^34 bytes), time= 38.7 seconds
					//  Test Name                         Raw       Processed     Evaluation
					//  [Low4/16]Gap-16:A                 R= +18.5  p =  2.5e-12    FAIL
				...
				Uint64 moremur64::raw64() {
					// named incorrectly, may name later... quarterback64 , due to use of 25 as a rotation?
...
//   ...and 1051 test result(s) without anomalies
					...
					// Another fail at 512GB... This time with a (very minor) "unusual" anomaly to boot.
					// The evaluation, at least, has one less exclamation mark after FAIL.
					// There were no anomalies before 512GB, at least.
					//rng=moremur64, seed=0x0
					//length= 512 gigabytes (2^39 bytes), time= 1253 seconds
					//  Test Name                         Raw       Processed     Evaluation
...
//   ...and 1157 test result(s) without anomalies
					...
					// Rotation 13 gets a variety of "unusual" anomalies, but passes 128TB.
// rng=plum256x13, seed=0x0
// length= 4 gigabytes (2^32 bytes), time= 5.5 seconds
//   Test Name                         Raw       Processed     Evaluation
//   [Low4/16]BCFN(2+1,13-1,T)         R=  -8.6  p =1-1.2e-4   unusual
//   ...and 687 test result(s) without anomalies
//
// rng=plum256x13, seed=0x0
// length= 2 terabytes (2^41 bytes), time= 2618 seconds
//   Test Name                         Raw       Processed     Evaluation
//   [Low4/16]Gap-16:B                 R=  +5.2  p =  2.4e-4   unusual
//   ...and 1019 test result(s) without anomalies
//
// rng=plum256x13, seed=0x0
// length= 4 terabytes (2^42 bytes), time= 5360 seconds
//   Test Name                         Raw       Processed     Evaluation
//   [Low1/32]BCFN(2+0,13-0,T)         R=  +9.0  p =  2.4e-4   unusual
//   ...and 1051 test result(s) without anomalies
					// Rotation 14 gets one "unusual" anomaly at 32GB, but otherwise passes 128TB:
// rng=plum256x14, seed=0x0
// length= 32 gigabytes (2^35 bytes), time= 43.6 seconds
//   Test Name                         Raw       Processed     Evaluation
...
//   ...and 982 test result(s) without anomalies
					// Rotation 17 passes 128TB with two unusual anomalies at 256GB:
// rng=plum256x17, seed=0x0
// length= 256 gigabytes (2^38 bytes), time= 335 seconds
//   Test Name                         Raw       Processed     Evaluation
//   [Low1/16]mod3n(5):(2,9-0)         R=  -8.7  p =1-3.6e-5   unusual
//   [Low8/64]BCFN(2+0,13-0,T)         R=  +9.6  p =  1.1e-4   unusual
//   ...and 913 test result(s) without anomalies
					// Rotation 18 passes 128TB with two "unusual" anomalies:
// rng=plum256x18, seed=0x0
// length= 2 gigabytes (2^31 bytes), time= 2.5 seconds
//   Test Name                         Raw       Processed     Evaluation
//   [Low8/32]FPF-14+6/16:all          R=  +5.3  p =  1.9e-4   unusual
//   ...and 645 test result(s) without anomalies
//
// rng=plum256x18, seed=0x0
// length= 32 gigabytes (2^35 bytes), time= 43.2 seconds
//   Test Name                         Raw       Processed     Evaluation
...
//   ...and 987 test result(s) without anomalies
					// Rotation 23 passes 128TB with three "unusual" anomalies:
// rng=plum256x23, seed=0x0
// length= 8 gigabytes (2^33 bytes), time= 11.5 seconds
//   Test Name                         Raw       Processed     Evaluation
//   [Low4/16]BCFN(2+0,13-0,T)         R=  -8.0  p =1-3.7e-4   unusual
//   ...and 729 test result(s) without anomalies
// rng=plum256x23, seed=0x0
// length= 128 gigabytes (2^37 bytes), time= 172 seconds
//   Test Name                         Raw       Processed     Evaluation
//   [Low8/64]BCFN(2+1,13-0,T)         R=  +9.9  p =  7.9e-5   unusual
//   ...and 879 test result(s) without anomalies
// rng=plum256x23, seed=0x0
// length= 512 gigabytes (2^39 bytes), time= 674 seconds
//   Test Name                         Raw       Processed     Evaluation
...
//   ...and 1179 test result(s) without anomalies
				...
				Uint64 overload320::raw64() {
					...
					// Goblin6Random
				    // Passes 128TB with no anomalies.
					// Period is 2 to the 384. Exactly 1D-equidistributed.
					uint64_t a = (stateA += 7777777777777777777UL);
					uint64_t b = (stateB += a + std::countl_zero(a));
					uint64_t c = (stateC += b + std::countl_zero(a &= b));
					uint64_t d = (stateD += c + std::countl_zero(a &= c));
					uint64_t e = (stateE += d + std::countl_zero(a &= d));
					uint64_t x = (stream += e + std::countl_zero(a &= e));
					x ^= x >> 27 ^ a;
					x *= 5555555555555555555UL;
					x ^= x >> 33 ^ b ^ c ^ d ^ e;
					x *= 3333333333333333333UL;
...
//   ...and 1019 test result(s) without anomalies
				...
				Uint16 floatHax32::raw16() {
					...
					float f = intBitsToFloat(
						((126u - std::countl_zero(rand())) << 23 & 0u - ((bits | 0u - bits) >> 31)) | (
							bits & 0x7FFFFF));
				...
				Uint16 floatHax64::raw16() {
					...
					float f = ldexpf((float) ((uint32_t) rand() | 0x80000001),
					                 -32 - std::countl_zero(proto_exp_offset));
				...
				Uint16 floatHaxPCG::raw16() {
					...
					uint32_t bits = (xorshifted >> rot) | (xorshifted << (32u - rot & 31u));
					float f = intBitsToFloat(
						          (126u - (uint32_t) std::countl_zero(
							           oldstate ^ (oldstate << rot | oldstate >> 64u - rot) ^ (
								           oldstate << 37u | oldstate >> 27u)) << 23) | (bits & 0x7FFFFFu) + 1u) -
					          2.7105058E-20f;
				...
				Uint32 doubleHaxFlow::raw32() {
					...
					double d = longBitsToDouble(
						(1022ULL - std::countl_zero(x) << 52 & 0ULL - (y != 0ULL)) | (y & 0xFFFFFFFFFFFFFULL));
